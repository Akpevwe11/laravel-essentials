...
class CreateHotelTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('discounts', function(Blueprint $table) {
            ...
            $table->string('code', 50)->comment('The code someone would be 
            expected to enter at checkout.'); 
        ...
        Schema::create('rates', function(Blueprint $table) {
         ...
         comment('The corresponding room type.');
        ...
        Schema::create('bookings', function(Blueprint $table) {
            $table->bigIncrements('id')->comment('The Primary Key for the table.');
            $table->unsignedBigInteger('room_id')->index('room_id')->comment('
            The corresponding room.'); 
            $table->date('start')->comment('The start date of the booking.'); 
            $table->date('end')->comment('The end date of the booking.'); 
            $table->boolean('is_reservation')->default(false)->comment('
            if this booking is a reservation.'); 
            $table->boolean('is_paid')->default(false)->comment('If this booking
            has been paid');
        ...
        Schema::create('bookings_users', function(Blueprint $table) {
            ...
            comment('The corresponding booking.'); 
            $table->unsignedBigInteger('user_id')->index('user_id')->
            comment('The corresponding user.'); 
            $table->timestamps(); 
